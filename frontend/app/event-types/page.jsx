...
function EventTypeModal({ eventType, onClose, onSave }) {
  ...
  const inputCls = (err) =>
    `w-full px-3 py-2 text-sm border rounded-lg outline-none transition-colors bg-[#1d1d1d] text-white
     ${err ? 'border-red-500 focus:border-red-500 focus:ring-2 focus:ring-red-950/40' : 'border-neutral-800 focus:border-violet-500 focus:ring-2 focus:ring-violet-950/40'}`;

  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4"
      onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-[#181818] border border-neutral-800/80 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col text-slate-100">
        {/* Header */}
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-neutral-800/80">
          <h2 className="text-lg font-bold text-slate-100">{isEdit ? 'Edit Event Type' : 'New Event Type'}</h2>
          <button onClick={onClose} className="p-1.5 hover:bg-neutral-800 rounded-lg transition-colors">
            <svg className="w-4 h-4 text-neutral-400 hover:text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </button>
        </div>
          ...
          {['general', 'questions'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors capitalize -mb-px border-b-2
                ${activeTab === tab ? 'text-violet-400 border-violet-400 font-semibold' : 'text-neutral-400 border-transparent hover:text-white'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {activeTab === 'general' && (
            <>
              {errors.submit && (
                <div className="bg-red-950/20 text-red-400 text-sm px-4 py-3 rounded-lg border border-red-900/30">{errors.submit}</div>
              )}
              <div>
                <label className="block text-xs font-semibold text-neutral-400 mb-1.5 uppercase tracking-wide">Title *</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-neutral-400 mb-1.5 uppercase tracking-wide">Description</label>
                <textarea className={inputCls()} rows={3} placeholder="Brief description..."
                  value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-neutral-400 mb-1.5 uppercase tracking-wide">Duration *</label>
                  <select className={inputCls(errors.duration)} value={form.duration} onChange={e => setForm(f => ({ ...f, duration: +e.target.value }))}>
                    {DURATIONS.map(d => <option key={d} value={d} className="bg-[#1d1d1d] text-white">{d} min</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-neutral-400 mb-1.5 uppercase tracking-wide">Location</label>
                  <select className={inputCls()} value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))}>
                    {LOCATIONS.map(l => <option key={l} value={l} className="bg-[#1d1d1d] text-white">{l}</option>)}
                  </select>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {['buffer_before', 'buffer_after'].map(field => (
                  <div key={field}>
                    ...
                    <select className={inputCls()} value={form[field]} onChange={e => setForm(f => ({ ...f, [field]: +e.target.value }))}>
                      {[0, 5, 10, 15, 20, 30].map(v => <option key={v} value={v} className="bg-[#1d1d1d] text-white">{v === 0 ? 'None' : `${v} min`}</option>)}
                    </select>
                  </div>
                ))}
              </div>
              <div>
                <label className="block text-xs font-semibold text-neutral-400 mb-2 uppercase tracking-wide">Color</label>
                <div className="flex flex-wrap gap-2">
                  {EVENT_COLORS.map(c => (
                    <button key={c} type="button" onClick={() => setForm(f => ({ ...f, color: c }))}
                       style={{ background: c }}
                       className={`w-7 h-7 rounded-full transition-transform hover:scale-110 ${form.color === c ? 'ring-2 ring-offset-2 ring-neutral-700 scale-110' : ''}`} />
                  ))}
                </div>
              </div>
              <label className="flex items-start gap-3 p-3 bg-[#242424] border border-neutral-800 rounded-lg cursor-pointer hover:bg-[#2c2c2c] transition-colors">
                ...
                <div>
                  <div className="text-sm font-medium text-slate-200">Requires Confirmation</div>
                  <div className="text-xs text-neutral-400 mt-0.5">Booking will be in "pending" state until you manually confirm it</div>
                </div>
              </label>
            </>
          )}
          ...
          {activeTab === 'questions' && (
            <div className="space-y-3">
              <p className="text-sm text-neutral-400">Add custom questions that bookers must answer when scheduling.</p>
              {questions.map((q, i) => (
                <div key={i} className="bg-[#242424] border border-neutral-800/80 rounded-xl p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Question {i + 1}</span>
                    <button onClick={() => removeQuestion(i)} className="text-xs text-red-500 hover:text-red-400 font-medium">Remove</button>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <select className={`${inputCls()} w-32`} value={q.type} onChange={e => updateQuestion(i, 'type', e.target.value)}>
                      <option value="text" className="bg-[#1d1d1d] text-white">Short text</option>
                      <option value="textarea" className="bg-[#1d1d1d] text-white">Long text</option>
                      <option value="phone" className="bg-[#1d1d1d] text-white">Phone</option>
                      <option value="select" className="bg-[#1d1d1d] text-white">Dropdown</option>
                      <option value="checkbox" className="bg-[#1d1d1d] text-white">Checkbox</option>
                    </select>
                  </div>
              ...
              <button onClick={addQuestion}
                className="w-full py-2.5 border-2 border-dashed border-neutral-800 rounded-xl text-sm text-neutral-400 hover:border-violet-500 hover:text-violet-400 transition-colors flex items-center justify-center gap-2">
  ...
  return (
    <div className="p-4 sm:p-8 max-w-4xl mx-auto fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-100">Event Types</h1>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 px-4 text-center bg-[#181818] border border-neutral-800 rounded-2xl">
          <div className="w-16 h-16 bg-[#242424] border border-neutral-800 rounded-2xl flex items-center justify-center mb-4">
            <svg className="w-8 h-8 text-neutral-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
          </div>
        ...
        <div className="space-y-3">
          {eventTypes.map(et => (
            <div key={et.id}
              className={`bg-[#181818] rounded-xl border border-neutral-800/80 flex overflow-hidden shadow-sm hover:shadow-md transition-all fade-in-up ${!et.is_active ? 'opacity-50' : ''}`}>
              {/* Color bar */}
              <div className="w-1.5 self-stretch flex-shrink-0" style={{ background: et.color }} />

              {/* Main Content Area */}
              <div className="flex-1 flex flex-col sm:flex-row sm:items-center justify-between p-4 sm:p-5 gap-4 min-w-0">
                {/* Info */}
                <div className="min-w-0">
                  <div className="font-semibold text-slate-100 text-sm flex items-center gap-2 flex-wrap">
                    <span>{et.title}</span>
                    {!et.is_active && (
                      <span className="bg-amber-500/10 text-amber-500 border border-amber-500/20 px-1.5 py-0.5 rounded text-[10px] font-bold tracking-wide uppercase">Hidden</span>
                    )}
                  </div>
                  <div className="flex items-center gap-x-3 gap-y-1.5 mt-1.5 flex-wrap">
                    ...
                    {(et.question_count || 0) > 0 && (
                      <span className="text-xs text-neutral-400 bg-[#242424] px-2 py-0.5 rounded-full border border-neutral-800">{et.question_count} question{et.question_count !== 1 ? 's' : ''}</span>
                    )}
                    {(et.buffer_after || 0) > 0 && (
                      <span className="text-xs text-neutral-400 bg-[#242424] px-2 py-0.5 rounded-full border border-neutral-800">+{et.buffer_after}m buffer</span>
                    )}
                  </div>
                  <div className="text-xs text-amber-500/80 mt-1.5 font-mono select-all truncate">/{et.username}/{et.slug}</div>
                </div>

                {/* Actions */}
                <div className="flex items-center gap-2 border-t border-neutral-800/60 pt-3 sm:border-0 sm:pt-0 justify-end flex-shrink-0">
                  {/* Toggle */}
                  <label className="toggle-switch mr-2" title={et.is_active ? 'Disable' : 'Enable'}>
                  ...
                  <div className="relative">
                    <button onClick={() => copyLink(et)}
                      title="Copy link"
                      className="p-2 hover:bg-neutral-800 rounded-lg transition-colors text-neutral-400 hover:text-white border border-neutral-800/60 sm:border-0">
                      {copiedId === et.id
                  ...
                  <button onClick={() => setDeleteConfirm(et)} title="Delete"
                    className="p-2 hover:bg-red-950/20 rounded-lg transition-colors text-neutral-400 hover:text-red-400 border border-neutral-800/60 sm:border-0">
